...
USERGRID_JAVA_OPTS="-server -Xms{{Xms}} -Xmx{{Xmx}}  -XX:MaxGCPauseMillis=500 -XX:GCPauseIntervalMillis=8000 -XX:ParallelGCThreads=8 -XX:ConcGCThreads=2 -XX:+UseG1GC -verbose:gc -XX:+PrintGCDetails -Xloggc:${LOGGING_PATH}easemob-push-gc.log -Dfile.encoding=utf-8 -Duser.language={{locale_lang}} -Duser.region={{locale_region}}"
